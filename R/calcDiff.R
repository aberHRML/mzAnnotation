...
function(mat,mo){
  if (nrow(mat) > 0) {
	  ...
	  if (mo == "n") {
      add <- adductRelationships[which(adductRelationships$Mode=="n"|adductRelationships$Mode=="b"),]
	  }
	  if (mo == "p") {
	  	add <- adductRelationships[which(adductRelationships$Mode=="p"|adductRelationships$Mode=="b"),]
	  }
	  cors <- mat[,1]
